			...
			<div class="row">
				...
				<div class="column middle">
					...
					<div class="jumbotron">
						<div class="row">
							<div class="col-sm-6">
                                <form action="/train/public/index/order" method="post">
									<p><label>手機號碼</label>
										<input type="text" name="phone" required> (必填)
                                    </p>
                                    <p><label>去程站</label>
                                    @foreach ($DATA as $data2)
										<input type="text" name="firststation" value="{{$data2->firststation}}">
                                    @endforeach
                                    </p>
                                    <p><label>抵達站</label>
                                    @foreach ($DATA as $data2)
										<input type="text" name="finalstation" value="{{$data2->finalstation}}">
                                    @endforeach
                                    </p>
									<p><label>搭乘日期</label>
										<input type="text" id="datepicker" name="ride_time" required>
									</p>
									<p><label>車次代碼</label>
										<input type="text" name="num" value="{{$num}}">	(必填) 
									</p> 
									<p><label>車票張數</label>
										<select name="t_amount" id="amount">
											@for($i=1;$i<=20;$i++)
												<option>{{$i}}</option>
											@endfor
										</select>
                                    </p>
									<p class=center><input type="submit" value="訂票" class="button2"></p>
                                    <!-- CSRF -->
									{!! csrf_field() !!}
                                </form>
