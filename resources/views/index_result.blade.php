            ...
            <div class="row">
                ...
                <div class="column middle">
	                ...
	                <div class="jumbotron">
						...
						<table border="1">
						<tr>
						<td>行駛星期</td><td>車種</td><td>列車車次</td><td>發車站</td><td>終點站</td><td>預計開車時間</td>
						<!--td>預計抵達時間</td--><td>行駛時間</td><td>票價</td><td>訂票</td>
						</tr>
						@foreach ($DATA as $data)
						<tr>
						<td>{{$data->week}}</td><td>{{$data->train_kind}}</td><td>{{$data->num}}</td><td>{{$data->firststation}}</td><td>{{$data->finalstation}}</td><td>{{$data->starttime}}</td>
						<!--td>預計抵達時間</td--><td>{{$data->s_time}}</td><td>{{$data->t_price}}</td>
						<td><a href="/train/public/index/order2/{{$data->num}}"><input type="button" value="訂票" class="button2"></a></td>
						</tr>
						@endforeach
                        {!! csrf_field() !!}
						</table>
