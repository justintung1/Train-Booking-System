...
@include('manage_menu')
  ...
  <div class="column middle">
    <h1>{{$title}}</h1>
	<form action="/01_Module_E/public/manage/train_add" method="post">
       <p><label>車種</label>
	...
	{!! csrf_field() !!}
	   ...
	   <table border="1">
	   <tr> <td>車種名稱</td> <td>最高時速</td> <td>編輯</td> <td>刪除</td>
	   @foreach ($DATA as $data)
	   <tr> <td>{{$data->name}}</td> <td>{{$data->speed}}</td> 
	   <td><a href="/01_Module_E/public/manage/train_edit/{{$data->kn}}">編輯</a></td> 
	   <td><a href="/01_Module_E/public/manage/train_del/{{$data->kn}}">刪除</a></td>
	   </tr>
	   @endforeach
	   </table>
